...
var CgiNodeConfig={Version:"0.2",StartTag:"<?",EndTag:"<?",ScriptExtensions:[".js"],EmbededScriptExtensions:[".jss"],SessionCookie:"CGI-NODE-SESSIONID",SessionTimeOut:900,SessionPath:process.env.CGI_NODE_SESSIONDIR||"/usr/lib/nodejs/sessions/"};
function CgiHttpContext(a){var b=this;this.__scripts=[];this.__vmContext=null;this.request=new CgiHttpRequest(a);this.response=new CgiHttpResponse;this.session=new CgiHttpSession(this.request,this.response);this.write=this.response.write;this.process=process;this.require=require;this.mapPath=function(a){var d=Path.dirname(b.request.server.path_translated);return Path.resolve(d,a)};this.include=function(a,d){void 0===d&&(d={encoding:"utf8"});var e=b.mapPath(a),g=FS.readFileSync(e,d);script=".js"!=
Path.extname(a)?CgiParser.script(b.__scripts.length,e,g.toString()):{id:b.__scripts.length,path:e,script:null,code:g,content:[]};b.__scripts.push(script);null===b.__vmContext&&(b.__vmContext=VM.createContext(b));VM.runInContext(script.code,b.__vmContext,script.path)};this.cgiNodeInfo=function(){var a=function(a,d){b.response.write('<tr><th colspan="2">'+a+"</th></tr>");for(var c in d){var f=d[c];if("function"!==typeof f){if("object"===typeof f){var k='<table class="NodeASPTable" border="0" style="margin: 0px">',
l;for(l in f)k+="<tr><td>"+l+"</td><td>"+f[l]+"</td></tr>";f=k+"</table>"}process.stdout.write("<tr><td>"+c+"</td><td>"+f+"</td></tr>")}}};b.response.write("<style>.Logo{ text-align: left; font-size: 36px !important; } .NodeASPTable{ font-family: arial; font-size: 12px; margin: auto; border-collapse: collapse; width: 600px} .NodeASPTable TH{ background-color: #303030; color: white; font-size: 14px; padding: 10px} .NodeASPTable TD{ padding: 5px; } .NodeASPTable TR TD:nth-child(1){ background: #d9ebb3; }</style>");
b.response.write('<table class="NodeASPTable" border="1">');b.response.write('<tr><th colspan="2" class="Logo">CGI-NODE v'+CgiNodeConfig.Version+"</th></tr>");var d={id:b.session.id,path:b.session.path,ipAddress:b.session.ipAddress};a("Node Versions",process.versions);a("CGI Command Line Arguments",process.argv);a("Server Variables",b.request.server);a("HTTP Request Headers",b.request.headers);a("HTTP Request Cookies",b.request.cookies);a("Session",d);a("Session Cookies",b.session.cookies);a("Session Data",
b.session.data);a("URL Query String",b.request.query);a("Post Form",b.request.post.form);a("Post Files",b.request.post.files);a("Post Parts",b.request.post.parts);b.response.write("</table>")}}
function CgiHttpSession(a,b){var c=this;this.ipAddress=this.path=this.id=null;this.cookies={};this.data={};this.init=function(){a.session=this;b.session=this;this.id=a.cookies.hasOwnProperty(CgiNodeConfig.SessionCookie)?a.cookies[CgiNodeConfig.SessionCookie]:this.create();var d=Path.join(CgiNodeConfig.SessionPath,this.id);FS.existsSync(CgiNodeConfig.SessionPath)||FS.mkdirSync(CgiNodeConfig.SessionPath,448);FS.existsSync(d)||(this.id=this.create());d=JSON.parse(FS.readFileSync(Path.join(CgiNodeConfig.SessionPath,
this.id)));if(d.ipAddress!=a.server.remote_addr)throw"Invalid session ID!";for(name in d)this[name]=d[name]};this.save=function(){FS.writeFileSync(c.path,JSON.stringify({id:c.id,path:c.path,ipAddress:c.ipAddress,cookies:c.cookies,data:c.data}))};this.create=function(){var b=a.server.remote_addr+a.server.remote_port+a.server.unique_id+(new Date).value+Math.random(),b=Crypto.createHash("md5").update(b).digest("hex"),c={id:b,path:Path.join(CgiNodeConfig.SessionPath,b),ipAddress:a.server.remote_addr,
cookies:{},data:{}};c.cookies[CgiNodeConfig.SessionCookie]={name:CgiNodeConfig.SessionCookie,value:b,httpOnly:!0,notSent:!0,server:!0};FS.writeFileSync(c.path,JSON.stringify(c));return c.id};this.cleanUp=function(){for(var a=(new Date).value,b=1E3*CgiNodeConfig.SessionTimeOut,c=FS.readdirSync(CgiNodeConfig.SessionPath),h=0;h<c.length;h++){var f=Path.join(CgiNodeConfig.SessionPath,c[h]);FS.statSync(f).mtime.value+b<a&&FS.unlinkSync(f)}};this.init()}
...
process.stdout.write(b.toString())};this.end=function(){a.sendHeaders();process.exit()}}
